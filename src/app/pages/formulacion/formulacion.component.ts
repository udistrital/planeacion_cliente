...
export class FormulacionComponent implements OnInit {
  ...
  loadPlanes() {
    this.request
      .get(environment.PLANES_CRUD, `plan?query=formato:true`)
      .subscribe(
        (data: any) => {
          if (data) {
  ...
  submit() {
    if (!this.banderaEdit) {
      // ADD NUEVA ACTIVIDAD
      if (
        this.dataArmonizacionPED.length != 0 &&
        this.dataArmonizacionPI.length != 0
      ) {
  ...
  mostrarIdentDocente(unidad: any): boolean {
    if (
      unidad.Id === 67 ||
      unidad.TipoDependencia.Id === 2 ||
      unidad.TipoDependencia === 2
    ) {
      return true;
    } else return false;
  }
  ...
  visualizeObs() {
    if (this.rol == 'JEFE_DEPENDENCIA') {
      ...
      if (
        this.estadoPlan == 'Formulado' ||
        this.estadoPlan == 'En revisión' ||
        this.estadoPlan == 'Revisado' ||
        this.estadoPlan == 'Ajuste Presupuestal'
      ) {
        this.readonlyObs = true;
    ...
    if (this.rol == 'PLANEACION') {
      ...
      if (
        this.estadoPlan == 'Pre Aval' ||
        this.estadoPlan == 'Aval' ||
        this.estadoPlan == 'Formulado'
      ) {
        this.readonlyObs = true;
        this.readOnlyAll = true;
  ...
  getVersiones(plan, planRecienCreado: boolean = false) {
    this.request
      .get(
        environment.PLANES_MID,
      ...
      .subscribe(
        (data: any) => {
          if (data) {
            ...
            this.plan = this.versiones[this.versiones.length - 1];
        ...
        .subscribe(
          (data: any) => {
            if (data !== null) {
              ...
              this.dataArmonizacionPED = strArmonizacion
                .split(',', len)
                .filter((item) => item != '');
    ...
    this.request
      ...
      .subscribe((data: any) => {
        if (data) {
          ...
          this.formArmonizacion
            .get('selectPED')
            .setValue(this.planesDesarrollo[0]);
          this.onChangePD(this.planesDesarrollo[0]);
        }
      });
  }
    ...
    this.request
      ...
      .subscribe((data: any) => {
        if (data) {
          ...
          this.formArmonizacion
            .get('selectPI')
            .setValue(this.planesIndicativos[0]);
          this.onChangePI(this.planesIndicativos[0]);
        }
      });
  }
  ...
  formularPlan() {
    this.request
      .post(
        environment.PLANES_MID,
        `formulacion/clonar_formato/${this.plan._id}`,
        {
          ...
          .subscribe(
            (data: any) => {
              if (data) {
                ...
                Swal.fire({
                  title:
                    'Error al versionar el plan. Por favor intente de nuevo',
                  icon: 'warning',
                  showConfirmButton: false,
                  timer: 2500,
                });
