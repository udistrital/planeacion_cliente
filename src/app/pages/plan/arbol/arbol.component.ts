...
export class ArbolComponent implements OnInit {
  ...
  loadArbolMid() {
    ...
    this.request.get(environment.PLANES_MID, `arbol/` + this.idPlan).subscribe(
      (data: any) => {
        if (data.Data !== null) {
          Swal.close();
          this.mostrar = true;
          ...
          Swal.close();
          Swal.fire({
            title: 'Error en plan de desarrollo',
            text: 'El Plan de Estratégico de Desarrollo seleccionado no esta correctamente creado, por favor elige otro.',
            icon: 'warning',
            showConfirmButton: false,
            timer: 7000
          });
  ...
  linksArbol() {
    ...
    this.treeControl.dataNodes.forEach((elementp, i) => {
      const idsHijos = this.treeControl.dataNodes
        .filter((elementh) => elementh.padre_idx == i)
        .map((e) => {
          return e.idx;
        });
  ...
  async armonizar(fila, bandera) {
    ...
    const idsArmo = this.treeControl.dataNodes
      .filter((elements) => elements.icon == Checked)
      .map((element) => {
        return element.id;
      });
      ...
      if (fila.activo != 'inactivo') {
          ...
          if (fila.nombre.toLowerCase().includes(No_Aplica)) {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                ...
                resolve(0); // No confirma dejar no aplica, aquí no se have nada
              }
            });
          } else {
        ...
        if (response != 0) {
          if (fila.padre_idx != undefined) {
            this.uncheckHijos(
              this.treeControl.dataNodes[fila.padre_idx].hijos_idx,
              uncheckNoAplicaOnly,
            );
          } else {
            this.treeControl.dataNodes
              .filter((ef) => ef.padre_idx == undefined)
              .forEach((e) => {
          ...
          const idxs = this.treeControl.dataNodes
            .filter((ef) => ef.level == 0)
            .map((e) => {
              return e.idx;
            });
  ...
  uncheckHijos(idHijos: number[], justForNoAplica?: boolean) {
    ...
    idHijos.forEach((id) => {
      if (!justForNoAplica) {
        ...
        this.treeControl.dataNodes[id].nombre.toLowerCase().includes(No_Aplica)
      ) {
        this.treeControl.dataNodes[id].icon = Unchecked;
      }
      this.uncheckHijos(
        this.treeControl.dataNodes[id].hijos_idx,
        justForNoAplica,
      );
    });
  }
  ...
  uncheckPadres(idPadre: number) {
    let thereIsMoreChecked = this.isAnotherChecked(
      this.treeControl.dataNodes[idPadre]?.hijos_idx,
    );
    while (idPadre != undefined && !thereIsMoreChecked) {
      this.treeControl.dataNodes[idPadre].icon = Unchecked;
      idPadre = this.treeControl.dataNodes[idPadre].padre_idx;
      thereIsMoreChecked = this.isAnotherChecked(
        this.treeControl.dataNodes[idPadre]?.hijos_idx,
      );
    }
  }
  ...
  unCheckNoAplicaHijosIfMulti(idxs: number[]) {
    ...
    for (let i = 0; i < idxs.length; i++) {
      if (this.treeControl.dataNodes[idxs[i]].icon == Checked) {
        this.unCheckNoAplicaHijosIfMulti(
          this.treeControl.dataNodes[idxs[i]].hijos_idx,
        );
        count++;
        if (
          this.treeControl.dataNodes[idxs[i]].nombre
            .toLowerCase()
            .includes(No_Aplica)
        ) {
          idNoAplica = idxs[i];
  ...
  expandNodes() {
    for (let nodo of this.dataArmonizacion) {
      let found = this.treeControl.dataNodes.find(
        (element) => element.id == nodo,
      );
